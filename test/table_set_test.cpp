...
TEST_F(TableSetWithSqliteConnectionTest, UpdateDatabaseAddsFields) {
  ...
  assert_string( "CREATE TABLE foo (id INTEGER PRIMARY KEY, bar INTEGER, baz TEXT, qux FLOAT, derp DATE)", sql );
}
...
TEST_F(TableSetWithSqliteConnectionTest, WithATableWithTheSameFieldsDoesNothing ) {
  connection.execute("CREATE TABLE foo (bar INTEGER);");
  TableSet schema = connection.schema();
